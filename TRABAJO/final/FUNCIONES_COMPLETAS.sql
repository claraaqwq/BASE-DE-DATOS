...
END;
/
EXEC INSERTAR_PERSONA('MARIO','MEDINA','C');
/
-------------------------------------------------------------------------------------------------
...
/
EXEC anadir_lote('QUESO',300);
/
SELECT * FROM HB_LOTE where nombre='QUESO';
-------------------------------------------------------------------------------------------------
...
--Proceso:
-------------------------------------------------------------------------------------------------
--11) PROCEDIMIENTOQUE QUITA VALORES DE MENOR LOTE
-------------------------------------------------------------------------------------------------
create or replace
...
execute at_colas(10,1001,10,20);
/
select * from hb_lote;
/
EXEC mover(10,1001);
...
select cantidad into cant2 from hb_lote where id_ingrediente=id_ingred and numero_lote=pos ;

if canti>0 AND CANT2=0   then
...
is 
    ...
    CURSOR NOR IS 
    select i.id_producto,i.id_ingrediente,i.cantidad
, nvl((select d.cant_producto
from hb_detalle_pedido D,HB_PRODUCTO PR
where PR.ID_PRODUCTO=d.id_producto and D.id_pedido=34566
and pr.tipo='N' and d.id_producto=i.id_producto),0)  as cantidad_pedidos
from hb_hamburguesa_ingrediente i
where i.id_producto in (select id_producto
from HB_detalle_pedido 
where id_producto in (select id_producto from hb_producto where tipo='N') AND ID_PEDIDO=34566);
