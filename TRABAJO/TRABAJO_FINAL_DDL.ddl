...
CREATE TABLE hb_cajero (
    ...
    hora_ingreso CHAR(8 BYTE) NOT NULL,
    hora_salida  CHAR(8 BYTE) NOT NULL
)
LOGGING;
...
ALTER TABLE hb_comprobante_metodo_pagov1 ADD CONSTRAINT hb_compr_met_pk PRIMARY KEY ( id_comprobante,
                                                                                      id_medio_pago );
...
CREATE TABLE hb_entrega (
    ...
    hora_entrega  CHAR(8 BYTE) NOT NULL
)
LOGGING;
...
ALTER TABLE hb_hamburguesa_ingrediente ADD CONSTRAINT hb_ham_ingre_pk PRIMARY KEY ( id_producto,
                                                                                    id_ingrediente );
...
CREATE TABLE hb_pedido (
    id_pedido   NUMBER NOT NULL,
    fecha       DATE NOT NULL,
    hora_pedido CHAR(8 BYTE) NOT NULL,
    id_cajero   NUMBER NOT NULL,
    numero      NUMBER,
...
COMMENT ON COLUMN hb_pedido.numero IS
    'Número del cliente si el pedido se da por delivery';

COMMENT ON COLUMN hb_pedido.direccion IS
    'Direccion del pedido si se dá por delivery';
...
ALTER TABLE hb_producto_combo ADD CONSTRAINT hb_combo_pk PRIMARY KEY ( id_combo,
                                                                       id_producto );

ALTER TABLE hb_pedido
    ADD CONSTRAINT hb_cajero_fk FOREIGN KEY ( id_cajero )
        REFERENCES hb_cajero ( id_cajero )
    NOT DEFERRABLE;

ALTER TABLE hb_comprobante_metodo_pagov1
    ADD CONSTRAINT hb_comprobante_fk FOREIGN KEY ( id_comprobante )
        REFERENCES hb_comprobante ( id_comprobante )
    NOT DEFERRABLE;

ALTER TABLE hb_hamburguesa_ingrediente
    ADD CONSTRAINT hb_ham_ingre_fk FOREIGN KEY ( id_ingrediente )
        REFERENCES hb_ingrediente ( id_ingrediente )
    NOT DEFERRABLE;

ALTER TABLE hb_lote
    ADD CONSTRAINT hb_ingrediente_fk FOREIGN KEY ( id_ingrediente )
        REFERENCES hb_ingrediente ( id_ingrediente )
    NOT DEFERRABLE;

ALTER TABLE hb_comprobante_metodo_pagov1
    ADD CONSTRAINT hb_metodo_pago_fk FOREIGN KEY ( id_medio_pago )
        REFERENCES hb_metodo_pago ( id_medio_pago )
    NOT DEFERRABLE;

ALTER TABLE hb_entrega
    ADD CONSTRAINT hb_pedido_fk FOREIGN KEY ( id_pedido )
        REFERENCES hb_pedido ( id_pedido )
    NOT DEFERRABLE;

ALTER TABLE hb_comprobante
    ADD CONSTRAINT hb_pedido_fkv2 FOREIGN KEY ( hb_pedido_id_pedido )
        REFERENCES hb_pedido ( id_pedido )
    NOT DEFERRABLE;

ALTER TABLE hb_detalle_pedido
    ADD CONSTRAINT hb_pedido_fkv3 FOREIGN KEY ( id_pedido )
        REFERENCES hb_pedido ( id_pedido )
    NOT DEFERRABLE;

ALTER TABLE hb_entrega
    ADD CONSTRAINT hb_persona_fk FOREIGN KEY ( id_motorizado )
        REFERENCES hb_persona ( id_persona )
    NOT DEFERRABLE;

ALTER TABLE hb_cajero
    ADD CONSTRAINT hb_persona_fkv2 FOREIGN KEY ( id_cajero )
        REFERENCES hb_persona ( id_persona )
    NOT DEFERRABLE;

ALTER TABLE hb_hamburguesa_ingrediente
    ADD CONSTRAINT hb_producto_fk FOREIGN KEY ( id_producto )
        REFERENCES hb_producto ( id_producto )
    NOT DEFERRABLE;

ALTER TABLE hb_detalle_pedido
    ADD CONSTRAINT hb_producto_fkv2 FOREIGN KEY ( id_producto )
        REFERENCES hb_producto ( id_producto )
    NOT DEFERRABLE;
